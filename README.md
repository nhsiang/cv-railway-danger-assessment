...
- ### YOLO semantic segmentation for rail curvature calculation
  ...
  - The median curvature, $`κ(y)_{\text{median}}`$, is used to determine the message on screen. If $`κ(y)_{\text{median}}>CURVATURE_THRESHOLD`$, the railway is curved and a warning message is displayed.
